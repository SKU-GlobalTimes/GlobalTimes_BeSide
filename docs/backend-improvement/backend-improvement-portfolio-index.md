...
이 문서는 GlobalTimes 백엔드 개선을 포트폴리오 관점에서 빠르게 선별하기 위한 인덱스다. 상세 수치와 재현 절차는 각 PR과 측정 문서가 원본이며, 여기서는 다음 연결만 압축한다.
...
실제 이력서에서는 프로젝트 설명 전체를 넣지 않고 직무와 가장 가까운 3~5개만 선택한다.
